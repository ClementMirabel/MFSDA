...
function Run_Callback(hObject, eventdata, handles)
    ...
    if isempty(ShapeDataName)
        warndlg('Please select the shape dataset folder by pressing "Shape Data" button.')
        return;
    end

    CoordDataName= get(handles.CoordData,'UserData');
    if isempty(CoordDataName)
        warndlg('Please select the coordinate data (template) folder by pressing "Coordinate Data" button')
        return;
    end
